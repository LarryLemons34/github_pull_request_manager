...
async function getPullR(gitUrl){
    ...
    if(typeof searchResult !== "undefined"){
        ...
        if(typeof filteredSearch[0] !== "undefined" && typeof filteredSearch[0].owner !== "undefined" && typeof filteredSearch[0].owner.login !== "undefined"){
            ...
            while(nextPage){
                // let pullRequests = await repo.listPullRequests({"state": "open", "page": pageNumber, "per_page": 100});//page_per_count
                let pullRequests = await paginationRequest(`https://api.github.com/repos/${filterResult.owner.login}/${filterResult.name}/pulls`)
                if(typeof pullRequests !== "undefined" && typeof pullRequests.data !== "undefined" && pullRequests.data.length > 0){
                    let promiseArray = pullRequests.data.map(async (pull) =>{
                        if(typeof pull !== "undefined"){
                            ...
                            allPullRequests.push({"pull_request": pull.title, "user" :pull.user.login, "commits": commits.length, "comments" : comments.length});
                        } else{
                            return false;
                        }
                    });
                    await Promise.all(promiseArray);
                    if(pullRequests.data.length < 100){ //page_per_count
                        nextPage = false
                    }
                    pageNumber++;
                } else {
